...
#TRACER_MC=3 #=2                   # Monte Carlo tracer particles (=1 to enable, >=2 to output as that partType)
...
#TRACER_PART_NUM_FLUID_QUANTITIES=3     # number of fluid quantities to be stored for velocity tracers - must match the value given to TRACER_PART_STORE_WHAT
#TRACER_PART_STORE_WHAT=8+16+32   # bit mask for quantities to store (see allvars.h for bitmask)
#TRACER_MC_NUM_FLUID_QUANTITIES=6       # number of fluid quantities to be stored for MC tracers - must match the value gien to TRACER_MC_STORE_WHAT
#TRACER_MC_STORE_WHAT=8+16+32+1024+2048+4096     # bit mask for quantities to store (see allvars.h for bitmask)
